...
const AppCard: React.FC<AppCardProps> = ({ app, allApps = [], onUpdate, onDelete, isAdmin = false }) => {
            ...
            {isRedundant && (app.alternatives?.length === 0) && (
              <div className="flex items-start md:items-center gap-2">
                {isEditingRec ? (
                  ...
                  <div
                    className={`group/rec flex items-start md:items-center gap-1.5 text-sm text-amber-700 font-medium bg-amber-50 px-2 py-1 rounded-md mt-1 border border-amber-100 w-fit transition-colors ${isAdmin ? 'cursor-pointer hover:bg-amber-100 hover:border-amber-200' : ''}`}
                    onClick={() => {
                      if (!isAdmin) return;
                      setRecValue(app.recommendation || '');
                      setIsEditingRec(true);
                    }}
                    title={isAdmin ? "Click to edit recommendation" : ""}
                  >
                    ...
                    {isAdmin && <Edit2 size={10} className="opacity-0 group-hover/rec:opacity-50 ml-1 transition-opacity" />}
                  </div>
                )}
              </div>
            )}
